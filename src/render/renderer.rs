...
impl<'a> Renderer<'a> {
    ...
    pub(crate) async fn new(window: &'a Window) -> Renderer<'a> {
    ...
    pub fn surface_configured(&self) -> bool {
        return self.surface_configured;
    }

    pub fn reinit_size(&mut self) {
        self.resize(self.size, true)
    }

    pub fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>, force: bool) {
        if force || (new_size.width > 0 && new_size.height > 0 && (
            self.size.width != new_size.width || self.size.height != new_size.height
        )) {
            self.size = new_size;
